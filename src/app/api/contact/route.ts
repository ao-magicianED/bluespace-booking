...
export async function POST(req: NextRequest) {
  ...
  const type = body.type === "longterm" ? "longterm" : "general";
  ...
  const typeLabel = type === "longterm" ? "長期・定期利用の相談" : "一般のお問い合わせ";
  ...
  await sendMail({
    to: email,
    subject: "【ブルーステージ】お問い合わせを受け付けました",
    text: [
      `${name} 様`,
      ``,
