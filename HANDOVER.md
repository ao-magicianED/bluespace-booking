...
⚠️ **2026-09-25: 0016/0017 が本番未適用だったと判明（同日適用・返金記録を補正済み）**（下表は0016まで。最新のファイル一覧は `supabase/migrations/` が正）。
再発防止として、コードが前提とするテーブル/列/RPCを `src/lib/schema-manifest.ts` に台帳化し、
毎日の `/api/cron/maintenance` が本番DBに存在するか確認して、無ければ管理者アラートを送る。
**マイグレーションを追加するときの手順は CLAUDE.md のチェックリストを参照**（台帳への追加を忘れると `npm test` が落ちる）。
